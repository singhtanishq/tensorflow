...
ENTRY main {
 ...
 ROOT while = (s32[]) while(param_0), condition=condition, body=body, backend_config={"known_trip_count":{"n":"10"}}
  ...
  // We expect that the nested while loop has been fully duplicated 10 times.
  // The inner while loop is copied 9 times (because it's a body), so there are
  // 10 inner while loops. The one outer while loop still remains so that's 11
  // while instructions. We check whether there are 22 distinct computations for
