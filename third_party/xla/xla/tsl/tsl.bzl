...
def if_hermetic_cuda_tools(if_true, if_false = []):
    ...
    return select({"@local_config_cuda//cuda:using_hermetic_cuda": if_true, "//conditions:default": if_false})  # copybara:comment_replace return if_false

def if_hermetic_cuda_libs(if_true, if_false = []):
    """Shorthand for select()'ing on whether we need to include hermetic CUDA libraries."""
    return select({"@local_config_cuda//cuda:include_cuda_libs": if_true, "//conditions:default": if_false})  # copybara:comment_replace return if_false
