...
void PopulateDefaultCalibrationOptions(QuantizationConfig& quant_config) {
  ...
  // By default, reuse calibration data if `calibration_data_dir` is set.
  if (!quant_config.calibration_options().has_reuse_calibration_data()) {
    quant_config.mutable_calibration_options()->set_reuse_calibration_data(
        !quant_config.calibration_options().calibration_data_dir().empty());
  }
}
