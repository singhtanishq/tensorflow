...
const std::string* g_string_legacy_model_path = nullptr;
...
void InitializeParams(
  ...
  if (graph_type == ModelGraphType::INT8) {
    ...
    graph_path = use_legacy_string_model ? *g_string_legacy_model_path
                                         : *g_string_model_path;
...
TEST(BenchmarkTest, DoesntCrashStringLegacyModel) {
  ASSERT_THAT(g_string_legacy_model_path, testing::NotNull());
...
int main(int argc, char** argv) {
  std::string fp32_model_path, int8_model_path, string_model_path,
      string_model_path_legacy, multi_signature_model_path;
  std::vector<tflite::Flag> flags = {
      ...
      tflite::Flag::CreateFlag(
          "string_graph_legacy", &string_model_path_legacy,
  ...
  g_string_legacy_model_path = &string_model_path_legacy;
